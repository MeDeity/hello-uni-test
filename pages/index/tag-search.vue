			...
			<scroll-view class="keyword-box" v-show="!isShowKeywordList" scroll-y>
				<view class="keyword-block" v-if="oldKeywordList.length>0">
					<view class="keyword-list-header">
						<view>历史搜索</view>
						<view>
							<image @tap="oldDelete" src="/static/img/delete.png"></image>
						</view>
					</view>
					<view class="keyword">
				...
				<view class="keyword-block">
					<view class="keyword-list-header">
						<view>热门搜索</view>
						<view>
							<image @tap="hotToggle" :src="'/static/img/attention'+forbid+'.png'"></image>
						</view>
					</view>
	...
	export default {
		...
		methods: {
			...
			loadOldKeyword() {
				uni.getStorage({
					...
					success: (res) => {
						...
						console.info("查询到历史数据:",JSON.stringify(this.oldKeywordList))
					}
				});
			},
			//加载热门搜索
			loadHotKeyword() {
				...
				console.info("查询到热门数据:",JSON.stringify(this.hotKeywordList))
				// getTop50TopicClassList().then(resp=>{
				// 	console.info("获取热门标签中",JSON.stringify(resp))
				// }).catch(error=>{
				// 	console.info("获取热门标签失败",error)
				// })
			}, 
			//监听输入
			inputChange(event) {
				...
				getTop50TopicClassList().then(resp=>{
					console.info("获取热门标签中",JSON.stringify(resp))
					this.keywordList = [];
					this.keywordList = this.drawCorrelativeKeyword(resp.data, keyword);
				}).catch(error=>{
					console.info("获取热门标签失败",error)
				})
			...
			drawCorrelativeKeyword(keywords, keyword) {
				...
				for (var i = 0; i < len; i++) {
					var row = keywords[i];
					//定义高亮#9f9f9f
					var html = row.className.replace(keyword, "<span style='color: #9f9f9f;'>" + keyword + "</span>");
					html = '<div>' + html + '</div>';
					var tmpObj = {
						keyword: row.className,
						htmlStr: html
					};
